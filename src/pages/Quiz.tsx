...
import { BookOpen, ArrowLeft, CheckCircle2, XCircle, Trophy, RotateCcw, Loader2 } from 'lucide-react';
...
export default function Quiz() {
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
        ...
        <Button onClick={() => navigate(topicId ? `/roadmap/${topicId}` : '/dashboard')}>
          Go Back
        </Button>
      </div>
    );
  ...
  const handleNext = async () => {
    if (currentIndex < questions.length - 1) {
      ...
      // Save quiz result
      if (user && topicId) {
        const finalScore = score + (selectedAnswer === currentQ.correctIndex ? 0 : 0); // score already updated
        try {
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 max-w-lg text-center">
          ...
          >
            ...
            <p className="text-muted-foreground text-lg mb-8">{topicTitle}</p>
            ...
            <div className="flex flex-col gap-3">
              <Button onClick={() => navigate(`/roadmap/${topicId}`)}>
                Back to Roadmap
              </Button>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-10 max-w-2xl">
        ...
        >
          <h2 className="text-xl md:text-2xl font-bold text-foreground mb-8 font-serif leading-relaxed">
            {currentQ.question}
          </h2>
            ...
            {currentQ.options.map((option, oi) => {
              ...
              if (showResult) {
                if (oi === currentQ.correctIndex) {
                  classes += 'bg-success/10 border-success/40 text-foreground';
                } else if (oi === selectedAnswer && oi !== currentQ.correctIndex) {
                  classes += 'bg-destructive/10 border-destructive/40 text-foreground';
                } else {
                  classes += 'bg-card border-border text-muted-foreground opacity-60';
                }
              } else {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-sm font-semibold shrink-0">
                      {String.fromCharCode(65 + oi)}
                    </span>
                    <span className="text-base">{option}</span>
                    {showResult && oi === currentQ.correctIndex && (
                      <CheckCircle2 className="h-5 w-5 text-success ml-auto shrink-0" />
                    )}
                    {showResult && oi === selectedAnswer && oi !== currentQ.correctIndex && (
